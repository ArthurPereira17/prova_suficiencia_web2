...
from service.userService import buscar_usuario_por_username, criar_usuario
from app.schemas import UsuarioCreate, UsuarioOut, Token
from lib.auth import hash_senha, verificar_senha, criar_access_token
...
def registrar(usuario: UsuarioCreate, db: Session = Depends(get_db)):
    if buscar_usuario_por_username(db, usuario.username):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Usuário já existe")
    novo = criar_usuario(db, usuario.username, hash_senha(usuario.senha))
    return novo
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    """
    usuario = buscar_usuario_por_username(db, form_data.username)
